...
def evaluate_elbo_on_test_set(model, test_loader, device):
    """
    ...
    Returns:
    elbo: [float]
        The average ELBO over the test set.
    """
    model.eval()
    total_elbo = 0
    with torch.no_grad():
...
def plot_aggregate_posterior(model, test_loader, device, n_components=2):
    ...
    plt.xlabel('PC1' if all_z.shape[1] > 1 else 'Latent Dim 1')
    plt.ylabel('PC2' if all_z.shape[1] > 1 else 'Latent Dim 2')
...
if __name__ == "__main__":
    ...
    elbo = evaluate_elbo_on_test_set(model, mnist_test_loader, device)
