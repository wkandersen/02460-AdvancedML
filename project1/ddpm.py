...
class DDPM(nn.Module):
    ...
    def negative_elbo(self, x):
        ...
        pred_noise = self.network(x_t, t.view(-1, 1).float() / self.T)
    ...
    def sample(self, shape):
        ...
        for t in range(self.T-1, -1, -1):
            ...
            pred_noise = self.network(x_t, torch.tensor([t], device=x_t.device).float() / self.T)
            x_t = (x_t - (1 - self.alpha[t]).sqrt() * pred_noise) / self.alpha[t].sqrt()
            if t > 0:
                noise = torch.randn_like(x_t)
                x_t += self.beta[t].sqrt() * noise
...
class FcNetwork(nn.Module):
    ...
    def forward(self, x, t):
        ...
        """
        # Ensure 't' matches the batch size of 'x'
